 ...
 * @requires SPServices 
 * @requires SharePoint 2013 or greater
...
var createData = {}
for (index in StratusFormsValuePairs)
{
	createData[StratusFormsValuePairs[index][0]] = StratusFormsValuePairs[index][1];
}
createData["StratusFormsFormId"] = ID;

var call = jQuery.ajax({
	url: _spPageContextInfo.webAbsoluteUrl + "/_api/Web/Lists/GetByTitle('"+ listName +"')/items" + IDQuery,
	type: "POST",
...
call.fail(function (jqHXR,textStatus,errorThrown){
	...
	+ textStatus + " - " + $(xData.responseXML).find("ErrorText").text());
	defer.resolve();
});
